<?php
    ...
    // Descripción del archivo. Código php de la página sanction.php. Realización de la consulta UPDATE e INSERT de las tablas actualiza y carnet.
    ...
    // Capturamos los datos provenientes del formulario de la página reward.php.

    $Codigo_curso = $_POST['grade'];
    $Num_exp = $_POST['student'];
    $Id_motivo = $_POST['reason'];

    $puntuation = "select Puntuacion as Puntuacion from perdida_ganancia where Id_motivo = '" . $Id_motivo . "' and Codigo_curso = '" . $Codigo_curso . "'";
    ...
    if (!empty($Codigo_curso) and !empty($Num_exp) and !empty($Id_motivo) and $_POST['reason'] != 'none' and $_POST['student'] != 'none'){
        ...
        header('Location: teachers.php?reward=y');
    
    } else
        header('Location: teachers.php?reward=no');
